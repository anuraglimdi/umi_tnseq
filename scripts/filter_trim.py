...
def filter_and_process_fastq(input_fastq, output_dir, transposon_seq, max_errors, trim_length, umi_length):
    ...
    index_file = os.path.join(output_dir, f"PF_{os.path.splitext(base_filename)[0]}.index")
    ...
    with open(input_fastq, 'r') as in_handle, \
         open(trimmed_fastq, 'w') as out_handle, \
         open(umi_file, 'w') as umi_handle:
